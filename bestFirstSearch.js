...
/**
 * Calculates a path between a start cell and an end cell using Best-First Search.
 */
function bestFirstSearch() {
  ...
  while(open.length > 0){
    ...
    var weight = evaluateCell(current_cell_node.address,end_cell)[HEURISTIC];
    current_cell_node.weight = weight;
    
    //update the order of operations
    order = operatorOrder(current_cell_node.address,end_cell);
    
    //visit current_cell
    description_map[current_cell_node.address[0]][current_cell_node.address[1]].visit();
    
    //adds all neighbors nodes on open queue
    evaluateNeighborsBestFirst(current_cell_node,order,description_map,height,width,end_cell,open);
    
    //sorts the open queue using only the heuristic weight
    open.sort(sortByHeuristicWeight);
 ...
 */
function evaluateNeighborsBestFirst(node,order,description_map,height,width,end_cell,open_array){
    ...
    switch(direction){
      case "up":
        ...
        if(isValidCellToVisit(neighbor,description_map,height,width)){
          ...
          var weight = evaluateCell(neighbor,end_cell)[HEURISTIC];
          var new_node = new SearchNode(0,0,neighbor,node.address,weight,0);
